...
const ACTIVE_FILTER_COLOR = '#2cd2f0';
...
export default function MonthlySaveReport() {
    ...
    const { data: reportData = [], isLoading, isError, error } = useQuery({
        queryKey: ['monthlyReport', queryParams],
        queryFn: () => api.getReport(queryParams),
        enabled: !!queryParams
    });
    ...
    const handleGenerate = () => {
        if (!month || !year || !operation) return;
        setFilters({ division: '', handledBy: '', status: '' });
        setFilterByDuplicate(false);
        setQueryParams({ month, year, operation });
    };
                ...
                <Box sx={{
                    ...
                    <Stack direction="row" spacing={1.5} alignItems="center" flexWrap="wrap" useFlexGap>
                        ...
                        <Button
                            ...
                            disabled={isLoading || !month || !year || !operation}
                            size="small"
                        ...
                        {isLoading ? (
                            ...
                            <TableRow>
                                <TableCell colSpan={8} align="center" sx={{ py: 3, color: 'text.secondary' }}>
                                    {queryParams?.month ? 'No data found for the selected filters.' : 'Select filters and click Generate.'}
                                </TableCell>
                            </TableRow>
                        )}
                    </TableBody>
