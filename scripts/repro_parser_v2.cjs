...
const splitLogData = (text) => {
    ...
    const initialSegments = text.split(hardSplitRegex).filter(s => s && s.trim().length > 10 && s.includes(':'));
    const finalSegments = [];
    initialSegments.forEach(segment => {
        ...
        lines.forEach((line, idx) => {
            ...
            if (match) {
                const cleanKey = match[1].trim().toLowerCase().replace(/^\s*\d+[\s\.)-]+\s*/, '').replace(/[^\w\s]/g, '').replace(/\s+/g, '_');
        ...
        const sessionFooters = lines.slice(lastRepeatIdx + 1).filter(line => line.match(/^\s*([^:]+):\s*(.*)$/));
...
const REQUIRED_KEYS = {
    staffAccess: ["Date_of_Request", "Branch_Code", "Concern_Category", "Concerning_Staff", "Staff_Name", "ID_Number"],
    camsAdjustment: ["Date_of_Request", "Branch_Code", "Concern_Category", "Concerning_Staff", "Client_Reference", "Client_Name", "Name_of_MFO", "MFO", "Group"],
    camsReopen: ["Date_of_Request", "Branch_Code", "Time_of_Request", "Adjustment_Point", "Requested_By", "Concern_Category", "Reason", "Recommended_By"],
    dailyCAMSConcerns: ["Date", "Branch_Code", "Concern", "Concerning_Staff", "Designation"]
};
...
segments.forEach((seg, i) => {
    const logType = identifyLogType(seg);
    const data = extractFields(seg);
    const mapping = {
        'date': 'date_of_request', 'date_of_request': 'date_of_request', 'group': 'group_name',
        'concern': 'concern_issue', 'id_number': 'id_number', 'corporate_email': 'corporate_email',
        'email': 'corporate_email', 'designation': 'designation', 'designate': 'designation',
        'client_ref': 'client_reference', 'client_reference': 'client_reference'
    };
    const dbData = {};
    Object.keys(data).forEach(k => {
        const lowerK = k.toLowerCase();
        const targetK = mapping[lowerK] || lowerK;
        dbData[targetK] = data[k];
    });

    const missing = logType ? REQUIRED_KEYS[logType].filter(key => {
        const actualKey = Object.keys(dbData).find(k => k.toLowerCase() === key.toLowerCase());
        return !actualKey || !dbData[actualKey];
    }) : [];

    console.log(\`Segment \${i+1} [Type: \${logType}]: \${missing.length > 0 ? "FAILED Validation (" + missing.join(', ') + ")" : "SUCCESS"}\`);
    if (dbData['corporate_email']) console.log(\`  - Captured Email: \${dbData['corporate_email']}\`);
});
